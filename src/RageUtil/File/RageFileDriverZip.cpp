...
bool
RageFileDriverZip::Load(RageFileBasic* pFile)
{
	ASSERT(m_pZip == NULL); /* don't load twice */
										...
										int& iCentralDirectoryOffset)
{
	...
	FileReading::read_16_le(*m_pZip,
							sError); /* skip number of entries on this disk */
	iTotalEntries = FileReading::read_16_le(*m_pZip, sError);
	FileReading::read_32_le(*m_pZip,
							sError); /* skip size of the central directory */
...
RageFileDriverZip::ParseZipfile()
{
	...
	for (int i = 0; i < iTotalEntries; ++i) {
		...
		FDB->AddFile("/" + pInfo->m_sName,
					 pInfo->m_iUncompressedSize,
					 pInfo->m_iCRC32,
					 pInfo);
	}
...
RageFileDriverZip::ProcessCdirFileHdr(FileInfo& info)
{
	...
	FileReading::read_16_le(*m_pZip,
							sError); /* relative offset of local header */
	FileReading::read_16_le(*m_pZip,
							sError); /* skip internal file attributes */
...
bool
RageFileDriverZip::ReadLocalFileHeader(FileInfo& info)
{
...
RageFileDriverZip::Open(const RString& sPath, int iMode, int& iErr)
{
	...
	auto* pSlice = new RageFileDriverSlice(
