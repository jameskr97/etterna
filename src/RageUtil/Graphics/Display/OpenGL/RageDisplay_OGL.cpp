...
struct GLPixFmtInfo_t {
	...
	{ /* R8G8B8A8 */ GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE, },
	{ /* R8G8B8A8 */ GL_RGBA8, GL_BGRA, GL_UNSIGNED_BYTE, },
	{ /* B4G4R4A4 */ GL_RGBA4, GL_RGBA, GL_UNSIGNED_SHORT_4_4_4_4, },
	{ /* B5G5R5A1 */ GL_RGB5_A1, GL_RGBA, GL_UNSIGNED_SHORT_5_5_5_1, },
	{ /* B5G5R5 */ GL_RGB5, GL_RGBA, GL_UNSIGNED_SHORT_5_5_5_1, },
	{ /* B8G8R8 */ GL_RGB8, GL_RGB, GL_UNSIGNED_BYTE, },
	{ /* Paletted */ GL_COLOR_INDEX8_EXT, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, },
	{ /* B8G8R8 */ GL_RGB8, GL_BGR, GL_UNSIGNED_BYTE, },
	{ /* A1R5G5B5 (matches D3DFMT_A1R5G5B5) */ GL_RGB5_A1, GL_BGRA, GL_UNSIGNED_SHORT_1_5_5_5_REV, },
	{ /* X1R5G5B5 */ GL_RGB5, GL_BGRA, GL_UNSIGNED_SHORT_1_5_5_5_REV, }
};
#pragma endregion

//// Translation Unit Classes ///////////////////////
#pragma region TU Classes
...
struct RageTextureLock_OGL : public RageTextureLock, public InvalidateObject {
  public:
	...
	void Lock(unsigned iTexHandle, RageSurface* pSurface) override {
		...
		const auto pSurfaceMemory =glMapBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB, GL_WRITE_ONLY);
...
#pragma region Translation Unit/Static Functions
static void CheckPalettedTextures() {
	std::string sError;
	do {
		...
		const auto glTexFormat   = g_GLPixFmtInfo[RagePixelFormat_PAL].internalfmt;
...
#define GL_CHECK_ERROR(f)                                                      \
	...
	{                                                                          \
		...
		glTexImage2D(GL_PROXY_TEXTURE_2D, 0, glTexFormat, 16, 16, 0,
...
static void CheckReversePackedPixels() {
	...
	if (glError == GL_NO_ERROR) {
		...
		Locator::getLogger()->trace("GL_UNSIGNED_SHORT_1_5_5_5_REV failed ({}), disabled", GLToString(glError).c_str());
	}
}
...
void RageDisplay_Legacy::setupGLShaders() {
	...
	// Needs more refactoring before we can move to ShaderGL object
	g_bTextureMatrixShader = ShaderGL::compileShader(GL_VERTEX_SHADER_ARB,"Data/Shaders/GLSL/Texture matrix scaling.vert");
	...
	// effects
	shaderBUnPreMultiply = ShaderGL{GL_FRAGMENT_SHADER,"Data/Shaders/GLSL/Unpremultiply.frag"};
	shaderBColorBurn = ShaderGL{GL_FRAGMENT_SHADER, "Data/Shaders/GLSL/Color burn.frag"};
	shaderBColorDodge = ShaderGL{GL_FRAGMENT_SHADER, "Data/Shaders/GLSL/Color dodge.frag"};
	shaderBVividLight = ShaderGL{GL_FRAGMENT_SHADER, "Data/Shaders/GLSL/Vivid light.frag"};
...
RageSurface* RageDisplay_Legacy::CreateScreenshot() {
	const auto width = window->getWindowDimensions().width; //(*g_pWind->GetActualVideoModeParams()).width;
	const auto height = window->getWindowDimensions().height; //(*g_pWind->GetActualVideoModeParams()).height;
...
//					 (*g_pWind->GetActualVideoModeParams()).height,
                     ...
                     window->getWindowDimensions().height, //
					 GL_RGBA,
					 GL_UNSIGNED_BYTE,
					 image->pixels);
		DebugAssertNoGLError();
...
intptr_t RageDisplay_Legacy::CreateRenderTarget(const RenderTargetParam& param, int& iTextureWidthOut, int& iTextureHeightOut) {
	/*
...
void RageDisplay_Legacy::DeleteTexture(intptr_t iTexture) {
	if (iTexture == 0)
		return;
...
RagePixelFormat RageDisplay_Legacy::GetImgPixelFormat(RageSurface*& img, bool& bFreeImg, int width, int height, bool bPalettedTexture) {
	auto pixfmt = FindPixelFormat(img->fmt.BitsPerPixel, img->fmt.Rmask, img->fmt.Gmask, img->fmt.Bmask, img->fmt.Amask);

	/* If img is paletted, we're setting up a non-paletted texture, and color indexes are too small, depalettize. */
	...
	if (!bSupported) {
		...
		const auto imgconv = CreateSurface(img->w, img->h, pfd->bpp,
                                           pfd->masks[0],pfd->masks[1],pfd->masks[2],
                                           pfd->masks[3]);
...
RageSurface* RageDisplay_Legacy::GetTexture(intptr_t iTexture) {
	...
	const int iFormat = iAlphaBits ? RagePixelFormat_RGBA8 : RagePixelFormat_RGB8;

	const auto& desc = PIXEL_FORMAT_DESC[iFormat];
	const auto pImage = CreateSurface(iWidth, iHeight, desc.bpp,desc.masks[0], desc.masks[1], desc.masks[2], desc.masks[3]);
...
void RageDisplay_Legacy::SetTexture(TextureUnit tu, intptr_t iTexture)
{
...
bool RageDisplay_Legacy::IsEffectModeSupported(EffectMode effect) {
	switch (effect) {
		...
		case EffectMode_Unpremultiply: return shaderBUnPreMultiply.isActive();
		case EffectMode_ColorBurn: return shaderBColorBurn.isActive();
		case EffectMode_ColorDodge: return shaderBColorDodge.isActive();
		case EffectMode_VividLight: return shaderBVividLight.isActive();
...
void RageDisplay_Legacy::SetMaterial(const RageColor& emissive, const RageColor& ambient, const RageColor& diffuse, const RageColor& specular, float shininess)
{
...
void RageDisplay_Legacy::SetRenderTarget(intptr_t iTexture, bool bPreserveTexture) {
	if (iTexture == 0) {
		g_bInvertY = false;
		glFrontFace(GL_CCW);
		...
		/* Reset the viewport. */
		const auto fWidth  = window->getWindowDimensions().width; //(*g_pWind->GetActualVideoModeParams()).windowWidth;
		const auto fHeight = window->getWindowDimensions().height; //(*g_pWind->GetActualVideoModeParams()).windowHeight;
...
void RageDisplay_Legacy::SetPolygonMode(PolygonMode pm) {
	...
	switch (pm) {
		...
		default: FAIL_M(ssprintf("Invalid PolygonMode: %i", pm));
	}
...
#pragma region Render Loop Related
...
#pragma region RageDisplay_Legacy Drawing Related
...
void RageDisplay_Legacy::DrawLineStripInternal(const RageSpriteVertex v[], int iNumVerts, float fLineWidth) {
	 ...
	 * window, we'll double the width. */
	{
		...
		const auto fWidthVal =
		  static_cast<float>(window->getWindowDimensions().width) / fW;
		const auto fHeightVal =
		  static_cast<float>(window->getWindowDimensions().height) /
		  fH;
...
void RageDisplay_Legacy::UpdateTexture(intptr_t iTexHandle, RageSurface* pImg, int iXOffset, int iYOffset, int iWidth, int iHeight) {
	glBindTexture(GL_TEXTURE_2D, iTexHandle);

	bool bFreeImg;
	const auto SurfacePixFmt = GetImgPixelFormat(pImg, bFreeImg, iWidth, iHeight, false);
	...
	glTexSubImage2D(GL_TEXTURE_2D, 0, iXOffset, iYOffset, iWidth, iHeight, glImageFormat, glImageType, pImg->pixels);
...
void RageCompiledGeometryHWOGL::Draw(int iMeshIndex) const {
	...
	GLboolean bLighting, bTextureGenS, bTextureGenT;
	glGetBooleanv(GL_LIGHTING, &bLighting);
