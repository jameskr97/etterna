...
class RageCompiledGeometry {
  public:
	...
	virtual void Change(const std::vector<msMesh>& vMeshes) = 0; /** @brief new data must be the same size as was passed to Set() */
	virtual void Draw(int iMeshIndex) const = 0;

  protected:
	[[nodiscard]] auto GetTotalVertices() const -> size_t {
		if (m_vMeshInfo.empty()) return 0;
...
auto RagePixelFormatToString(RagePixelFormat i) -> const std::string&;
...
class VideoModeParams {
  public:
	...
	VideoModeParams(bool windowed_, std::string sDisplayId_, int width_, int height_, int bpp_, int rate_,
					bool vsync_, bool interlaced_, bool bSmoothLines_, bool bTrilinearFiltering_,
					bool bAnisotropicFiltering_, bool bWindowIsFullscreenBorderless_, std::string sWindowTitle_,
					std::string sIconFile_, bool PAL_, float fDisplayAspectRatio_)
	  : windowed(windowed_), sDisplayId(std::move(sDisplayId_)), width(width_), height(height_), bpp(bpp_),
	    rate(rate_), vsync(vsync_), interlaced(interlaced_), bSmoothLines(bSmoothLines_)
	  , bTrilinearFiltering(bTrilinearFiltering_), bAnisotropicFiltering(bAnisotropicFiltering_)
	  , bWindowIsFullscreenBorderless(bWindowIsFullscreenBorderless_), sWindowTitle(std::move(sWindowTitle_))
	  , sIconFile(std::move(sIconFile_)), PAL(PAL_), fDisplayAspectRatio(fDisplayAspectRatio_)
	{}

	VideoModeParams(const VideoModeParams& other) : windowed(other.windowed), sDisplayId(other.sDisplayId),
	    width(other.width), height(other.height), bpp(other.bpp),
	    rate(other.rate), vsync(other.vsync), interlaced(other.interlaced), bSmoothLines(other.bSmoothLines),
	    bTrilinearFiltering(other.bTrilinearFiltering), bAnisotropicFiltering(other.bAnisotropicFiltering),
...
class ActualVideoModeParams : public VideoModeParams {
  public:
	...
	ActualVideoModeParams(const VideoModeParams& params) : VideoModeParams(params),
	    windowWidth(params.width), windowHeight(params.height){}
...
class RageDisplay {
  ...
  public:
	...
	[[nodiscard]] virtual auto GetPixelFormatDesc(RagePixelFormat pf) const -> const RagePixelFormatDesc* = 0;
	...
	[[nodiscard]] virtual auto GetActualVideoModeParams() const -> const ActualVideoModeParams* = 0;
	...
	virtual auto IsEffectModeSupported(EffectMode effect) -> bool { return effect == EffectMode_Normal; }

	[[nodiscard]] virtual auto SupportsRenderToTexture() const -> bool { return false; }
	[[nodiscard]] virtual auto SupportsFullscreenBorderlessWindow() const -> bool { return false; }
	...
	virtual auto CreateRenderTarget(const RenderTargetParam& /*unused*/, int& /* iTextureWidthOut */,
	 ...
	 */
	virtual void SetRenderTarget(intptr_t /* iHandle */, bool /* bPreserveTexture */ = true) {}
	...
	[[nodiscard]] virtual auto GetTextureDiagnostics(unsigned /* id */) const -> std::string { return std::string(); }

	virtual auto CreateScreenshot() -> RageSurface* = 0; // allocates a surface.  Caller must delete it.
	virtual auto GetTexture(intptr_t /* iTexture */) -> RageSurface* { return nullptr; } // allocates a surface.  Caller must delete it.
  ...
  public:
	...
	auto FindPixelFormat(int bpp, unsigned Rmask, unsigned Gmask, unsigned Bmask, unsigned Amask,
