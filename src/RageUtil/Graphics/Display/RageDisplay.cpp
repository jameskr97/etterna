...
#include "RageUtil/Graphics/Display/Surface/RageSurface_Save_BMP.h"
#include "RageUtil/Graphics/Display/Surface/RageSurface_Save_JPEG.h"
#include "RageUtil/Graphics/Display/Surface/RageSurface_Save_PNG.h"
...
static int g_iFramesRenderedSinceLastCheck, g_iFramesRenderedSinceLastReset, g_iVertsRenderedSinceLastCheck, g_iNumChecksSinceLastReset;
...
void RageDisplay::ProcessStatsOnFlip() {
	if (PREFSMAN->m_bShowStats || LOG_FPS) {
		...
		const std::chrono::duration<double> timeDelta = std::chrono::steady_clock::now() - g_LastCheckTimer;
...
struct Centering {
	explicit Centering(int iTranslateX = 0, int iTranslateY = 0, int iAddWidth = 0, int iAddHeight = 0)
	  : m_iTranslateX(iTranslateX), m_iTranslateY(iTranslateY), m_iAddWidth(iAddWidth), m_iAddHeight(iAddHeight) {}
...
std::string RageDisplay::SetVideoMode(VideoModeParams p, bool& bNeedReloadTextures) {
	...
	p.sDisplayId = d.id();
	const auto supported = d.currentMode() != nullptr ? *d.currentMode() : *d.supportedModes().begin();
...
RageDisplay::~RageDisplay() { LUA->UnsetGlobal("DISPLAY"); } // Unregister with Lua.

const RageMatrix* RageDisplay::GetCentering() const { return &g_CenteringMatrix; }
const RageMatrix* RageDisplay::GetProjectionTop() const { return g_ProjectionStack.GetTop(); }
const RageMatrix* RageDisplay::GetViewTop() const { return g_ViewStack.GetTop(); }
const RageMatrix* RageDisplay::GetWorldTop() const { return g_WorldStack.GetTop(); }
const RageMatrix* RageDisplay::GetTextureTop() const { return g_TextureStack.GetTop(); }
void RageDisplay::PushMatrix() { g_WorldStack.Push(); }
void RageDisplay::PopMatrix() { g_WorldStack.Pop(); }
void RageDisplay::Translate(float x, float y, float z) { g_WorldStack.TranslateLocal(x, y, z); }
...
void RageDisplay::TextureTranslate(float x, float y) { g_TextureStack.TranslateLocal(x, y, 0); }
...
void RageDisplay::ResolutionChanged() { UpdateCentering(); }
...
RageMatrix RageDisplay::GetCenteringMatrix(float fTranslateX, float fTranslateY, float fAddWidth, float fAddHeight) {
	...
	// edge = 1
	const auto fWidth = static_cast<float>((*GetActualVideoModeParams()).windowWidth);
	const auto fHeight = static_cast<float>((*GetActualVideoModeParams()).windowHeight);
...
RageMatrix RageDisplay::GetOrthoMatrix(float l, float r, float b, float t, float zn, float zf) {
	return RageMatrix(2 / (r - l),0,0,0, 0, 2 / (t - b), 0, 0, 0, 0, -2 / (zf - zn), 0,
...
#pragma region RageCompiledGeometry
...
RageSurface* RageDisplay::CreateSurfaceFromPixfmt(RagePixelFormat pixfmt, void* pixels, int width, int height, int pitch) {
