...
void
ConvertLittleEndian24BitToFloat(void* pBuf, int iSamples)
{
	/* Convert in reverse, so we can do it in-place. */
...
struct WavReaderPCM : public WavReader
{
	...
	bool Init() override
	{
		if (QuantizeUp(m_WavData.m_iBitsPerSample, 8) < 8 ||
			...
			m_sError = ssprintf("Unsupported sample size %i",
								m_WavData.m_iBitsPerSample);
			return false;
		}

		if (m_WavData.m_iFormatTag == 3 && m_WavData.m_iBitsPerSample != 32) {
			m_sError = ssprintf("Unsupported float sample size %i",
								m_WavData.m_iBitsPerSample);
			return false;
		}
	...
	{
		const int iBytesPerSec = m_WavData.m_iSampleRate *
  ...
  public:
	...
	bool Init() override
	{
		if (m_WavData.m_iBitsPerSample != 4) {
			m_sError = ssprintf("Unsupported ADPCM sample size %i",
								m_WavData.m_iBitsPerSample);
			return false;
		}
	...
	{
		ASSERT_M(m_iBufferUsed == m_iBufferAvail,
				 ssprintf("%i", m_iBufferUsed));
...
RageSoundReader_WAV::Open(RageFileBasic* pFile)
{
	...
	while (!bGotFormatChunk || !bGotDataChunk) {
		...
		if (ChunkID == "fmt ") {
			...
			m_WavData.m_iBitsPerSample =
			  FileReading::read_16_le(*m_pFile, sError);
			m_WavData.m_iExtraFmtBytes =
			  FileReading::read_16_le(*m_pFile, sError);

			if (m_WavData.m_iChannels < 1 || m_WavData.m_iChannels > 2)
				FATAL_ERROR(ssprintf("Unsupported channel count: %i",
									 m_WavData.m_iChannels));
	...
	switch (m_WavData.m_iFormatTag) {
		...
		case 2: // ADPCM
			...
			break;
		case 85: // MP3
			/* Return unknown, so other decoders will be tried.  MAD can read
			 * MP3s embedded in WAVs. */
			return OPEN_UNKNOWN_FILE_FORMAT;
		default:
			FATAL_ERROR(
...
int
RageSoundReader_WAV::GetLength() const
{
	ASSERT(m_pImpl != NULL);
...
int
RageSoundReader_WAV::SetPosition(int iFrame)
{
	ASSERT(m_pImpl != NULL);
...
int
RageSoundReader_WAV::GetNextSourceFrame() const
{
	ASSERT(m_pImpl != NULL);
...
int
RageSoundReader_WAV::Read(float* pBuf, int iFrames)
{
	ASSERT(m_pImpl != NULL);
