...
GetSSCNoteData(const Song& song, const Steps& in, bool bSavingCache)
{
	...
	lines.emplace_back(ssprintf(
	  "#MSDVALUES:%s;", NotesWriterSSC::MSDToString(in.GetAllMSD()).c_str()));
	lines.emplace_back(
	...
	if (bSavingCache) {
		...
		lines.emplace_back(song.m_vsKeysoundFile.empty() ? "#NOTES:"
														 : "#NOTES2:");
		...
		lines.insert(lines.end(),
					 ...
					 std::make_move_iterator(splitData.end()));
		lines.emplace_back(";");

		// cause of the decompress call above- maybe unnecessary because who's
		// going to resync a file and not play it? -mina
		in.Compress();
	}
