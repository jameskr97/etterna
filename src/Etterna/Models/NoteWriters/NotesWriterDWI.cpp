...
NotesToDWIString(TapNote tnCol1, TapNote tnCol2, TapNote tnCol3, TapNote tnCol4)
{
	return NotesToDWIString(
	  tnCol1, TAP_EMPTY, tnCol2, tnCol3, TAP_EMPTY, tnCol4);
}
...
static void
WriteDWINotesField(RageFile& f, const Steps& out, int start)
{
	NoteData notedata;
	...
	for (int m = 0; m <= iLastMeasure; m++) // foreach measure
	{
		...
		for (double b = fFirstBeatInMeasure;
			 b <= fLastBeatInMeasure - 1 / 64.0f;
			 b += fCurrentIncrementer) // need the -0.0001 to account for
									   // rounding errors
		{
			...
			switch (out.m_StepsType) {
				...
				case StepsType_dance_solo:
					str = NotesToDWIString(notedata.GetTapNote(0, row),
					...
					break;
				default:
					FAIL_M(ssprintf("StepsType not supported by DWI: %i",
									out.m_StepsType));
			}
			f.Write(str);
		}
...
static bool
WriteDWINotesTag(RageFile& f, const Steps& out)
{
...
NotesWriterDWI::Write(const std::string& sPath, const Song& out)
{
	...
	if (!stops.empty()) {
		...
		for (unsigned i = 0; i < stops.size(); i++) {
			...
			f.Write(ssprintf("%.3f=%.3f",
							 fs->GetRow() * 4.0f / ROWS_PER_BEAT,
