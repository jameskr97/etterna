...
void
ActorMultiVertex::LoadFromNode(const XNode* Node)
{
	std::string path;
	Node->GetAttrValue("Texture", path);
	if (!path.empty() &&
		!TEXTUREMAN->IsTextureRegistered(RageTextureID(path))) {
...
void
ActorMultiVertex::UnloadTexture()
{
	if (_Texture != nullptr) {
...
void
ActorMultiVertex::SetNumVertices(size_t n)
{
	if (n == 0) {
...
void
ActorMultiVertex::DrawPrimitives()
{
	...
	if (m_pTempState->diffuse[0] != RageColor(1, 1, 1, 1) &&
		...
		for (auto& vertice : TS.vertices) {
			...
			// float, multiplied by the channel from the RageColor, then the
			// result converted to uint8_t.  If implicit conversion is allowed
			// to happen, sometimes the compiler decides to turn the RageColor
			// into a uint8_t, which makes any value other than 1 into 0.  Thus,
			// the explicit conversions.  -Kyz
#define MULT_COLOR_ELEMENTS(color_a, color_b)                                  \
	color_a = static_cast<uint8_t>(static_cast<float>(color_a) * (color_b));
			...
			MULT_COLOR_ELEMENTS(vertice.c.b, m_pTempState->diffuse[0].b);
			MULT_COLOR_ELEMENTS(vertice.c.r, m_pTempState->diffuse[0].r);
			MULT_COLOR_ELEMENTS(vertice.c.g, m_pTempState->diffuse[0].g);
			MULT_COLOR_ELEMENTS(vertice.c.a, m_pTempState->diffuse[0].a);
...
CubicSplineN*
ActorMultiVertex::GetSpline(size_t i)
{
...
void
ActorMultiVertex::SetSecondsIntoAnimation(float seconds)
{
	SetState(0);
	if (_Texture != nullptr) {
...
void
ActorMultiVertex::SetCurrentTweenStart()
{
	AMV_start = AMV_current;
}

void
ActorMultiVertex::EraseHeadTween()
{
...
void
ActorMultiVertex::UpdatePercentThroughTween(float PercentThroughTween)
{
...
void
ActorMultiVertex::BeginTweening(float time, ITween* pTween)
{
...
void
ActorMultiVertex::FinishTweening()
{
	if (!AMV_Tweens.empty()) {
...
void
ActorMultiVertex::AMV_TweenState::MakeWeightedAverage(
  AMV_TweenState& average_out,
...
class LunaActorMultiVertex : public Luna<ActorMultiVertex>
{
  public:
	...
	static void SetVertexFromStack(T* p,
								   lua_State* L,
								   size_t VertexIndex,
								   int DataStackIndex)
	{
		// Use the number of arguments to determine which property a table is
		// for
		if (lua_type(L, DataStackIndex) != LUA_TTABLE) {
			LuaHelpers::ReportScriptErrorFmt(
			  "ActorMultiVertex::SetVertex: non-table parameter supplied. "
		...
		for (size_t i = 0; i < NumDataParts; ++i) {
			...
			if (DataPieceElements == 2) {
				...
				// don't need to.
				RageColor c;
				// Does not use FromStackCompat because we are not compatible
				// with passing a color in non-table form.
	...
	static int SetVertices(T* p, lua_State* L)
	{
		...
		if (lua_type(L, 1) == LUA_TNUMBER) {
			...
			if (First < 0) {
				LuaHelpers::ReportScriptErrorFmt(
				  "ActorMultiVertex::SetVertices: index %d provided, cannot "
				  "set Index < 1",
				  First + 1);
				COMMON_RETURN_SELF;
			}
	...
	static void FillStateFromLua(lua_State* L,
...
#define DATA_ERROR(i)                                                          \
	if (!lua_istable(L, i)) {                                                  \
		luaL_error(L,                                                          \
				   "The state data must be in a table like this: {{left, "     \
				   "top, right, bottom}, delay}");                             \
	}
