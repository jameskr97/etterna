...
void HandleInputEvents(float fDeltaTime); // Has external usages
...
//	LOG = new RageLog;
    ...
    if (!g_bAllowMultipleInstances.Get() && archHooks->CheckForMultipleInstances(argc, argv)) {
        ShutdownGame();
        return 0;
    }
...
// Functions Defined within source only
namespace StepMania {
    ...
    void ApplyGraphicOptions() {
        ...
        bNeedReload |= MODELMAN->SetPrefs(ModelManagerPrefs(PREFSMAN->m_bDelayedModelDelete));
    ...
    std::string GetInitialScreen() {
        if (PREFSMAN->m_sTestInitialScreen.Get() != "" && SCREENMAN->IsScreenNameValid(PREFSMAN->m_sTestInitialScreen)) {
...
// Anonymous Namespace for local functions only
namespace {
    static void update_centering() {
    ...
    static void StartDisplay() {
        ...
        TEXTUREMAN->SetPrefs(RageTextureManagerPrefs(PREFSMAN->m_iTextureColorDepth, PREFSMAN->m_iMovieColorDepth,
                                        PREFSMAN->m_bDelayedTextureDelete, PREFSMAN->m_iMaxTextureResolution,true,
                                        PREFSMAN->m_bForceMipMaps));
    ...
    bool HandleGlobalInputs(const InputEventPlus& input) {
        ...
        bool bIsShiftHeld =
          INPUTFILTER->IsBeingPressed(DeviceInput(DEVICE_KEYBOARD, KEY_LSHIFT), &input.InputList) ||
