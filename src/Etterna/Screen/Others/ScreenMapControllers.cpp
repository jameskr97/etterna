...
#define MAPPED_TO_COMMAND(gc, slot)                                            \
	THEME->GetMetricA(m_sName,                                                 \
					  ssprintf("MappedToP%iS%iCommand", (gc) + 1, (slot) + 1))
	...
	for (unsigned b = 0; b < m_KeysToMap.size(); b++) {
		...
		FOREACH_ENUM(GameController, c)
		{
			for (int s = 0; s < NUM_SHOWN_GAME_TO_DEVICE_SLOTS; s++) {
				...
				pKey->m_textMappedTo[c][s]->RunCommands(
				  MAPPED_TO_COMMAND(c, s));
				ActorUtil::LoadAllCommands(*pKey->m_textMappedTo[c][s],
										   m_sName);
...
#define ADD_ACTION(index, a, b, c, d, e)                                       \
	...
	ADD_ACTION(0,
			   m_sName,
			   "Clear",
			   &ScreenMapControllers::ClearToDefault,
			   m_Line.back(),
			   &m_LineScroller);
	ADD_ACTION(1,
			   m_sName,
			   "Reload",
			   &ScreenMapControllers::ReloadFromDisk,
			   m_Line.back(),
			   &m_LineScroller);
	ADD_ACTION(2,
			   m_sName,
			   "Save",
			   &ScreenMapControllers::SaveToDisk,
			   m_Line.back(),
			   &m_LineScroller);
	ADD_ACTION(3,
			   m_sName,
			   "SetList",
			   &ScreenMapControllers::SetListMode,
			   m_Line.back(),
			   &m_LineScroller);
	ADD_ACTION(4,
			   m_sName,
			   "Exit",
			   &ScreenMapControllers::ExitAction,
			   m_Line.back(),
			   &m_LineScroller);
#undef ADD_ACTION
	...
	m_LineScroller.SetNumItemsToDraw(
	  static_cast<float>(m_LineScroller.GetNumChildren()) * 2);
...
void
ScreenMapControllers::Update(float fDeltaTime)
{
		...
		m_DeviceIToMap.IsValid()) // we're going to map an input
	{
		...
		GameInput curGameI(static_cast<GameController>(m_CurController),
						   pKey->m_GameButton);
 ...
 * use. */
static bool
IsAxis(const DeviceInput& DeviceI)
{
	if (!DeviceI.IsJoystick())
		return false;

	static int axes[] = { JOY_LEFT,
						  JOY_RIGHT,
						  JOY_UP,
						  JOY_DOWN,
						  JOY_LEFT_2,
...
bool
ScreenMapControllers::Input(const InputEventPlus& input)
{
	if (m_fLockInputSecs > 0.0f) {
		return false;
	}
	...
	// Whoever wants it can uncomment this log spew, I don't think it's
	// necessary. -Kyz LOG->Trace( "ScreenMapControllers::Input():  device: %d,
	// button: %d", 	input.DeviceI.device, input.DeviceI.button );
	...
	if (!m_WaitingForPress.IsZero()) {
		...
		if (input.DeviceI.device == DEVICE_KEYBOARD &&
			(input.DeviceI.button >= KEY_F1 &&
			 input.DeviceI.button <= KEY_F12)) {
		...
		switch (button) {
				/* We only advertise space as doing this, but most games use
				 * either delete or backspace, and I find them more intuitive,
				 * so allow them, too. -gm */

				/* XXX: For some reason that eludes me, this function gets sent
				 * an KEY_SPACE button press every time the JOY_HAT_UP button is
				 * pressed. Had to put this in to prevent mappings being erased
				 * everytime the user pressed up on the joypad. */
			...
			case KEY_BACK: // Clear the selected input mapping.
				...
				}
				{
					const KeyToMap* pKey = &m_KeysToMap[CurKeyIndex()];
					GameInput curGameI(
					  static_cast<GameController>(m_CurController),
					  pKey->m_GameButton);
	...
	// LOG->Trace( "m_CurSlot: %d m_CurController: %d m_CurButton: %d",
	// m_CurSlot, m_CurController, m_CurButton );
...
Actor*
ScreenMapControllers::GetActorWithFocus()
{
	if (CursorOnAction()) {
...
void
ScreenMapControllers::BeforeChangeFocus()
{
...
void
ScreenMapControllers::AfterChangeFocus()
{
	Actor* pActor = GetActorWithFocus();
...
void
ScreenMapControllers::DismissWarning()
{
	m_Warning->PlayCommand("TweenOff");
}

bool
ScreenMapControllers::CursorOnAction()
{
...
bool
ScreenMapControllers::CursorOnHeader()
{
...
bool
ScreenMapControllers::CursorOnKey()
{
	return !(CursorOnHeader() || CursorOnAction());
}

bool
ScreenMapControllers::CursorCanGoUp()
{
	return m_CurButton > 0;
}
...
bool
ScreenMapControllers::CursorCanGoLeft()
{
	return !CursorOnAction() && (m_CurSlot > 0 || m_CurController > 0);
}

bool
ScreenMapControllers::CursorCanGoRight()
{
...
int
ScreenMapControllers::CurKeyIndex()
{
...
int
ScreenMapControllers::CurActionIndex()
{
...
void
ScreenMapControllers::SetCursorFromSetListCurrent()
{
...
void
ScreenMapControllers::StartWaitingForPress()
{
	const KeyToMap* pKey = &m_KeysToMap[CurKeyIndex()];
	BitmapText* pText = pKey->m_textMappedTo[m_CurController][m_CurSlot];
...
void
ScreenMapControllers::HandleMessage(const Message& msg)
{
